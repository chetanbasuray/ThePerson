...
class Person:
    ...
    def introduce(self) -> None:
        ...
        if not self.hobbies.is_hobbyless():
            parts.append(f"In my free time, I enjoy {self.hobbies.summary()}.")
    ...
    def play(self) -> None:
        """Pick a random hobby and has fun.

        This method chooses a hobby from the person's hobby list and
        prints a message. If the person has too many hobbies (is_adhd),
        they may become distracted.
        """
        if self.hobbies.is_hobbyless():
            self.say("I don't have any hobbies.")
            return

        hobby = self.choose(self.hobbies.list)
        self.say(f"I'm currently busy with my favorite hobby: {hobby.name}!")
        
        # If they have too many hobbies, they are probably distracted.
        if self.hobbies.is_adhd():
            self.say("Wait... what was I doing again? I have too many hobbies!")
